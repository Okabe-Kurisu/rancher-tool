...
def get_icon(chart_name_str, pkg_name_str):
    ...
    with open(chart_name_str, encoding='utf-8') as chart:
        ...
        if not img_url:
            no_icon_list.append(pkg_name_str)
            return
    ...
    print(pkg_name_str + "'s logo has been downloaded already\n")
...
def find_all_chart():
    ...
    for file_name in os.listdir(config['path']):
        ...
        for pkg_name in os.listdir(config['path'] + file_name + "/"):
            chart_name = config['path'] + file_name + "/" + pkg_name + "/Chart.yaml"
            if os.path.isfile(chart_name):
                get_icon(chart_name, config['path'] + file_name + "/")
    if not no_icon_list:
